    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-[var(--bg-input)] border border-[var(--border)] rounded-lg p-3 shadow-xl">
          ...
          <p className="text-sm text-[var(--accent-orange)]">Signal: {vals.signal?.toFixed(4)}</p>
        ...
        {!loading && !error && data.length > 0 && (
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={data} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
              ...
              <ReferenceLine y={0} stroke="var(--text-muted)" strokeOpacity={0.2} />
              <Bar dataKey="histogram" fill="var(--accent-green)" fillOpacity={0.53} />
              <Line type="monotone" dataKey="macd" stroke="var(--accent-blue)" strokeWidth={1.5} dot={false} />
              <Line type="monotone" dataKey="signal" stroke="var(--accent-orange)" strokeWidth={1.5} dot={false} />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      </div>
